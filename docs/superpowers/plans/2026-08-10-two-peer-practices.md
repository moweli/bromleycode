...
**Tech Stack:** Next.js 16.3 App Router, React 19.2, TypeScript, Tailwind CSS v4 (CSS-first `@theme`), `sharp` for image encoding. Verification uses `fetch` only, with Playwright as an optional upgrade for the two layout checks — it is **not** a dependency of this repo and must not become one.
...
Expected FAILs, and only these:

- `/services/data-platform-engineering responds` and `/services/data-ai-strategy responds`, which arrive in Tasks 3 and 4.
- The two redirect checks, which arrive in Task 4.
- `does not claim "ten stages"` on `/`, `/services` and `/how-we-work`, which Tasks 5 to 7 replace.

Seven expected failures in total. Every other check should PASS.

The layout checks will print `SKIP` unless Playwright happens to be installed. That is correct behaviour, not a failure, and the script must still exit 0 when only skips remain.

If anything other than those four fails, that is a pre-existing defect: record it and stop.
